...
> Как мгновенно найти пункт об индексации в 340+ договорах, не тратя выходные на чтение сканов?
...
Стандартный RAG из туториала сломался на первом же реальном документе. Одно и то же лицо встречалось в пяти формах: «ИП Лыкова», «Лыкова В.В.», «Лыкова» — один арендатор или три? Векторный поиск не умеет отвечать «сколько платит Балаян?» — он ищет похожий текст, а не точные цифры.

Это история о том, **как выглядит RAG когда встречается с реальностью**.
...
ask("Сколько платит Пластамак?")
# → [SQL] найдено: 10 800 руб./мес. из актов Excel

ask("Сколько платит Радал?")
# → [SQL] пусто → авто-fallback → [RAG] ищет в тексте договора

ask("Условия расторжения с Милославской")
# → [RAG + фильтр] только документы Милославской, ноль чужих договоров
...
### 🔴 Challenge 1: Проблема «Евгения Викторовича»

Natasha NER находила имена — но возвращала **директора арендатора**, а не самого арендатора. «Гладских Евгений Викторович» — подписант от имени ООО «Точка Опоры», не арендатор.
...
`RecursiveCharacterTextSplitter` режет все документы на чанки по 1500 символов. Для финансового акта это катастрофа: «Заказчик: ИП Пластамак» в одном чанке, «Итого к оплате: 10 800» — в другом. Модель видит цифру без контекста.
...
Без фильтрации при вопросе про Милославскую в контекст попадали договоры Балаяна, счета Радала и квитанции с Мира 284. LLM путалась и давала неверные ответы.
...
⏳  Уровень 6    Cohere Reranker  ← инфраструктура готова
